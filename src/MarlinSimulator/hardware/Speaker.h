...
class Speaker : public VirtualPrinter::Component {
public:
  Speaker(pin_type pin, bool invert_logic) : VirtualPrinter::Component("Speaker"), pin(pin), invert_logic(invert_logic) {
    ...
    audio_set_sample_cb([this](void* userdata, Uint8* sample_buffer, int samples_buffer_size){
      ...
      nanos_per_sample *= Kernel::TimeControl::realtime_scale;

      for (uint32_t i = 0; i < sample_count; i++) {
        if (buzzer_pin_was_high((last_cb_sim_time) + (i * nanos_per_sample))) {
          samples[i] = INT16_MAX * m_volume; // wave only above midpoint
        } else {
          samples[i] = 0;
        }
      }
